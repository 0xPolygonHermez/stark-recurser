...
describe("Linear Hash Circuit Test", function () {
    ...
    before( async() => {
        template = await fs.promises.readFile(path.join(__dirname, "circom", "linearhash.bn128.test.circom.ejs"), "utf8");
    });
    ...
    it("Should calculate linear hash of 9 complex elements and arity 16", async () => {
        ...
        circuit = await wasm_tester(circuitFile, {O:1, verbose:false, include: ["src/pil2circom/circuits.bn128", "node_modules/circomlib/circuits"]});
    ...
    it("Should calculate linear hash of 110 complex elements with arity 4", async () => {
        ...
        const lh = new LinearHash(poseidon, 4, false);
