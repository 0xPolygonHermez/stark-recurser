...
function getBits(idx, nLevels, arity) {
    const logArity = Math.ceil(log2(arity));
    res = [];
...
describe("Merkle Hash BN128 Circuit Test Custom", function () {
    ...
    it("Should calculate merkle hash of 9 complex elements with arity 16", async () => {
        ...
        const circuit = await wasm_tester(circuitFile, {O:1, include: ["src/pil2circom/circuits.bn128", "node_modules/circomlib/circuits"]});
